// github.com/comiccoin-network/monorepo/cloud/comiccoin/internal/nameservice/service/claimcoins/claimcoins.go
package hello
...
type claimCoinsServiceImpl struct {
	...
	getNameServicePrivateKeyService                         svc_faucet.GetNameServicePrivateKeyService
...
func NewClaimCoinsService(
	...
	return &claimCoinsServiceImpl{
		...
		getNameServicePrivateKeyService:                         getNameServicePrivateKeyService,
...
func (svc *claimCoinsServiceImpl) Execute(sessCtx mongo.SessionContext) (*ClaimCoinsResponse, error) {
	...
	privateKey, err := svc.getNameServicePrivateKeyService.Execute(sessCtx)
	if err != nil {
	...
	remoteAccount, err := svc.fetchRemoteAccountBalanceFromAuthorityUseCase.Execute(sessCtx, svc.config.Blockchain.NameServiceAccountAddress)
	if err != nil {
	...
	if remoteAccount.Balance < svc.config.Blockchain.NameServiceClaimCoinsReward {
	...
	tx := &dom_auth_tx.Transaction{
		...
		From:       svc.config.Blockchain.NameServiceAccountAddress,
		To:         user.WalletAddress,
		Value:      svc.config.Blockchain.NameServiceClaimCoinsReward + svc.config.Blockchain.TransactionFee, // Note: The transaction fee gets reclaimed by the Authority, so it's fully recirculating when authority calls this.
		Data:       []byte{},
	...
	claim := &dom_user.UserClaimedCoinTransaction{
		ID:        primitive.NewObjectID(),
		Timestamp: time.Now(),
		Amount:    svc.config.Blockchain.NameServiceClaimCoinsReward,
	}
	user.ClaimedCoinTransactions = append(user.ClaimedCoinTransactions, claim)

	// Increment the total coins claimed by user.
	user.TotalCoinsClaimed += svc.config.Blockchain.NameServiceClaimCoinsReward
	...
	faucet.TotalCoinsDistributed += svc.config.Blockchain.NameServiceClaimCoinsReward + svc.config.Blockchain.TransactionFee
	faucet.TotalTransactions += 1

	// Update daily counters
	faucet.TotalCoinsDistributedToday += svc.config.Blockchain.NameServiceClaimCoinsReward + svc.config.Blockchain.TransactionFee

// cloud/comiccoin/internal/iam/interface/http/publicwalletdirectory/listbyfilter.go
...
type ListPublicWalletsFromDirectoryByFilterHTTPHandler interface {
	Handle(w http.ResponseWriter, r *http.Request)
}

type listPublicWalletsFromDirectoryByFilterHTTPHandlerImpl struct {
	config  *config.Configuration
	logger  *slog.Logger
	db      *mongo.Client
	service svc.ListPublicWalletsFromDirectoryByFilterService
}

func NewListPublicWalletsFromDirectoryByFilterHTTPHandler(
	config *config.Configuration,
	logger *slog.Logger,
	db *mongo.Client,
	service svc.ListPublicWalletsFromDirectoryByFilterService,
) ListPublicWalletsFromDirectoryByFilterHTTPHandler {
	return &listPublicWalletsFromDirectoryByFilterHTTPHandlerImpl{
		config:  config,
		logger:  logger,
		db:      db,
		service: service,
...
func (h *listPublicWalletsFromDirectoryByFilterHTTPHandlerImpl) Handle(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

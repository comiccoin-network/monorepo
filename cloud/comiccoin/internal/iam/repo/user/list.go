// github.com/comiccoin-network/monorepo/cloud/comiccoin/internal/iam/repo/user/list.go
package user
...
func (s *userStorerImpl) hasActiveFilters(filter *dom_user.UserFilter) bool {
	return filter.Name != nil ||
		...
		filter.Status != 0 ||
		filter.Role != 0 ||
		filter.ProfileVerificationStatus != 0 ||
...
func (impl *userStorerImpl) ListByFilter(ctx context.Context, filter *dom_user.UserFilter) (*dom_user.UserFilterResult, error) {
	...
	if err != nil {
		impl.Logger.Warn("Failed to get total count", slog.Any("error", err))

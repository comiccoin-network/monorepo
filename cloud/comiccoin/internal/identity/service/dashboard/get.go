// github.com/comiccoin-network/monorepo/cloud/comiccoin/internal/identity/service/dashboard/get.go
package dashboard
...
func (svc *getDashboardServiceImpl) Execute(sessCtx mongo.SessionContext) (*DashboardDTO, error) {
	...
	// //
	//
	// identity, err := svc.getFaucetByChainIDUseCase.Execute(sessCtx, svc.config.Blockchain.ChainID)
	// if err != nil {
	// 	svc.logger.Error("failed getting identity by chain id error", slog.Any("err", err))
	// 	return nil, err
	// }
	// if identity == nil {
	// 	err := fmt.Errorf("identity d.n.e. for chain ID: %v", svc.config.Blockchain.ChainID)
	// 	svc.logger.Error("failed getting identity by chain id error", slog.Any("err", err))
	// 	return nil, err
	// }
	...
	return &DashboardDTO{
		...
		// FaucetBalance:           identity.Balance,

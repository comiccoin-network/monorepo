...
#   4. Run the compiled binary after successful build (-command="./comiccoin daemon")
#   5. Suppress log prefix for cleaner output (-log-prefix=false)
ENTRYPOINT CompileDaemon -polling=true -log-prefix=false -build="/go/bin/quality-build.sh" -command="./comiccoin daemon" -directory="./"

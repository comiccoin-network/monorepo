...
class NFTTransferService {
    ...
    async getTransactionTemplate(senderAddress, recipientAddress, amount, message = '', tokenID, tokenMetadataURI) {
        try {
            ...
            // Developers Note:
            // See: comiccoin-authority/service/tx/prepare.go
            ...
            const requestPayload = {
                ...
                data: '',
                type: 'token',
                token_id_string: tokenID.toString(), // NFT specific field.
                token_metadata_uri: tokenMetadataURI, // NFT specific field.
            }
    ...
    async createStamp(transaction) {
        ...
        const orderedObj = {
            ...
            token_id_bytes: transaction.token_id_bytes, // NFT specific field.
            token_metadata_uri: transaction.token_metadata_uri, // NFT specific field.
            token_nonce_bytes: transaction.token_nonce_bytes, // NFT specific field.
            type: 'token', // NFT specific field.
        ...
        console.log(
            'Initial JSON bytes:',
            Array.from(ethers.toUtf8Bytes(initialJson)).map((b) => b.toString(16).padStart(2, '0'))
        )
        ...
        return ethers.getBytes(ethers.keccak256(ethers.concat([stampBytes, messageBytes])))
    }
    ...
    async signTransaction(template) {
        try {
            ...
            const v = this.comicNFTId + recoveryBit // 29 or 30
            ...
            // Developers note:
            // Part of security in our blockchain to prevent reply attacks by
            // incrementing the `nonce` of the NFT for every transaction that
            // involves NFT transfers. For web-based wallets we want to have this
            // value automatically computed via the `getTransactionTemplate`
            // function.
            //
            ...
            const signedTx = {
                ...
                data_string: '',
                type: 'token', // NFT specific field.
                token_id_bytes: template.token_id_bytes, // NFT specific field.
                token_id_string: '',
                token_metadata_uri: template.token_metadata_uri, // NFT specific field.
                token_nonce_bytes: template.token_nonce_bytes, // NFT specific field.
                token_nonce_string: '',
                v_bytes: [v],
    ...
    async verifySignature(signedTransaction) {
        try {
            ...
            const recoveryBit = comicNFTV - this.comicNFTId // Should be 0 or 1

...
const BurnNFTPage = () => {
  ...
  useEffect(() => {
    console.log('BurnNFTPage: Initial useEffect running');
    let mounted = true;

    const checkWalletSession = async () => {
      console.log('BurnNFTPage: checkWalletSession starting');
      try {
        if (!mounted) return;
        setIsLoading(true);

        if (serviceLoading) {
          console.log('BurnNFTPage: Service still loading, waiting...');
          return;
        }

        if (!currentWallet) {
          console.log('BurnNFTPage: No current wallet found, redirecting to login');
          if (mounted) {
            setForceURL("/login");
          }
          return;
        ...
        console.error('BurnNFTPage: Session check error:', error);
  ...
  if (forceURL !== "" && !serviceLoading) {
    console.log('BurnNFTPage: Navigating to:', forceURL);

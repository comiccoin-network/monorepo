...
const ShareButton = ({ tokenMetadataUri, metadata, tokenId }) => {
  ...
  const getButtonStyles = () => {
    ...
    switch (shareState) {
      ...
      default: return `${baseStyles} text-gray-500 hover:text-gray-700 hover:bg-gray-100 active:bg-gray-200`;
    }
  };
...
const DownloadButton = ({ onClick, icon: Icon, label, variant = 'primary' }) => {
  ...
  const getButtonStyles = () => {
    const baseStyles = "flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-200 touch-manipulation";
    const variantStyles = {

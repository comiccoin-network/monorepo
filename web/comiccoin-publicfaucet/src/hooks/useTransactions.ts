...
export function useTransactions({
     ...
     * IMPORTANT: Using empty dependency array to prevent infinite loop
     */
    const fetchTransactions = useCallback(async (force = false): Promise<Transaction[]> => {
        ...
        try {
            ...
            // This will use our customAxios with token refresh
            ...
            const errorObj = err instanceof Error ? err : new Error(err?.message || 'Failed to fetch transactions')
    ...
    useEffect(() => {
        ...
        console.log(`⏰ TRANSACTIONS HOOK: Setting up refresh interval of ${refreshInterval}ms`)

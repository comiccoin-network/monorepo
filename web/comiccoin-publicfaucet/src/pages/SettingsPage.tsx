...
// Form field interface for type-checking
interface FormField {
  id: string;
  label: string;
  type: string;
  name: keyof UpdateMeRequestDTO;
  placeholder: string;
  required?: boolean;
  disabled?: boolean;
...
const SettingsPageContent: React.FC = () => {
  ...
  // Prevent iOS scroll bounce and optimize for mobile devices
  useEffect(() => {
  ...
  useEffect(() => {
    if (statusMessage.type && statusRef.current) {
    ...
    switch (name) {
      ...
      case 'last_name':
        if (value && value.length < 2) {
          errorMessage = `${name === 'first_name' ? 'First' : 'Last'} name must be at least 2 characters`;
        }
        break;
      case 'phone':
  ...
  // Define form fields for better organization
    ...
    >
      ...
      <header className="mb-6 md:mb-8">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-purple-800" id="settings-heading">Account Settings</h1>
